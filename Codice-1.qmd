...
tema_base <- theme_minimal(base_family = "lato", base_size = 13) +
  theme(
    plot.title       = element_markdown(family = "playfair", size = 18,
                                        face = "bold", color = "#1a1a2e",
                                        margin = margin(b = 6)),
    plot.subtitle    = element_markdown(size = 12, color = "#555577",
                                        margin = margin(b = 16)),
...
ggplot(df_heat, aes(x = Year, y = Paese, fill = Value)) +
  ...
  geom_text(
    ...
    color  = if_else(df_heat$Value > 45 | is.na(df_heat$Value), "white", "#222222"),
    size   = 3.2,
    family = "lato",
    na.rm  = FALSE
  ) +
